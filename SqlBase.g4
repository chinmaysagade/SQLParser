...
statement
    ...
    | ALTER TABLE table=multipartIdentifier
        RENAME COLUMN
        from1=multipartIdentifier TO to=errorCapturingIdentifier        #renameTableColumn
    | ALTER TABLE multipartIdentifier
        DROP (COLUMN | COLUMNS)
        '(' columns=multipartIdentifierList ')'                        #dropTableColumns
    | ALTER TABLE multipartIdentifier
        DROP (COLUMN | COLUMNS) columns=multipartIdentifierList        #dropTableColumns
    | ALTER (TABLE | VIEW) from1=multipartIdentifier
    ...
    | ALTER TABLE multipartIdentifier
        from1=partitionSpec RENAME TO to=partitionSpec                  #renameTablePartition
    | ALTER (TABLE | VIEW) multipartIdentifier
...
primaryExpression
    ...
    | POSITION '(' substr=valueExpression IN str1=valueExpression ')'                           #position
    ...
    | (SUBSTR | SUBSTRING) '(' str1=valueExpression (FROM | ',') pos=valueExpression
      ((FOR | ',') len1=valueExpression)? ')'                                                   #substring
    | TRIM '(' trimOption=(BOTH | LEADING | TRAILING)? (trimStr=valueExpression)?
       FROM srcStr=valueExpression ')'                                                         #trim
    | OVERLAY '(' input1=valueExpression PLACING replace=valueExpression
...
unitToUnitInterval
    : value=intervalValue from1=identifier TO to=identifier
    ;
...
dataType
    : complex_type=ARRAY '<' dataType '>'                            #complexDataType
    | complex_type=MAP '<' dataType ',' dataType '>'                 #complexDataType
    | complex_type=STRUCT ('<' complexColTypeList? '>' | NEQ)        #complexDataType
